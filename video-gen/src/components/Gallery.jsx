...
import { Download, AlertCircle, Loader2, Play, Clock, ChevronDown } from 'lucide-react'

function VideoCard({ video }) {
  const [expanded, setExpanded] = useState(false)

  return (
    <div className="bg-surface-1 border border-white/8 rounded-2xl overflow-hidden group hover:border-white/15 transition-all">
      {/* Video / placeholder */}
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center gap-3 text-xs text-white/30 font-mono">
          ...
          <span>·</span>
          <span className="capitalize">{video.params.model}</span>
        </div>
        ...
        {expanded && (
          <p className="text-xs text-white/30 italic">{video.params.negative_prompt}</p>
        )}
